...
class ux_block(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(f'x_perm after norm:{x.shape}')
        x = self.pwconv1(x)
        #print(f'x_pw_1: {x.shape}')
        x = self.act(x)
        #print(f'x_act:{x.shape}')
        x = self.pwconv2(x)
        #print(f'x_pw_2:{x.shape}')
        x = x.permute(0, 2, 3, 4, 1)
        #print(f'x_perm_after_pw:{x.shape}')
        if self.gamma is not None:
            x = self.gamma * x

        x = x.permute(0, 4, 1, 2, 3)
        #print(f'x_perm_final:{x.shape}')
